...
export function PaletteButton({
  ...
  return (
    <button
      ...
      className={cn(
        "relative flex size-9 items-center justify-center rounded-sm border text-sm font-semibold transition-transform",
        statusClass[status],
    ...
    <ul className="grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
      {items.map((i) => (
        <li key={i.key} className="flex items-center gap-2">
          <span
            className={cn(
              "relative flex size-6 shrink-0 items-center justify-center rounded-sm border text-[11px] font-semibold",
              statusClass[i.key],
            )}
          >
            {counts[i.key]}
